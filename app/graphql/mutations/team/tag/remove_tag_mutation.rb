module Mutations
  module Team
    module Tag
      RemoveTagMutation = GraphQL::Field.define do
        type Types::TagType
        ...
        authorize! ->(tag, _args, ctx) {
          ::Team::TagPolicy.new(ctx[:current_account], tag).remove?
        }

        resolve RemoveTagMutationResolver.new
      end

      class RemoveTagMutationResolver
        def call(_obj, args, ctx)
          ...
          action = ::Team::Tag::Remove.run(inputs)
          if action.valid?
            action.result
          else
            action.errors
